...
module providing a primitive "house" shape.  We'll walk through the
entire process of creating, testing, and submitting such an extension.
I encourage you to follow along, either by copying and pasting the
code shown here, or you can just use it as a guideline for making your
own extension.
...
this tutorial, feel free to ask on the [XXX](diagrams mailing list),
...
sources using [http://darcs.net/ darcs]. `darcs` is a distributed
revision control system, written in Haskell, which is used to track
the source code for xmonad and the xmonad-contrib extension library.
If you already have the darcs sources, you can skip this section.
...
Next, create a directory where you will store the xmonad repositories.
 ...
 [brent@archimedes:~/haskell]$ mkdir diagrams-dev-tut
 [brent@archimedes:~/haskell]$ cd diagrams-dev-tut/
 [brent@archimedes:~/haskell/diagrams-dev-tut]$ 

Now, use <code>darcs get</code> to create new directories and download the
current repositories into them:

 [brent@archimedes:~/haskell/xmonad-dev-tut]$ darcs get http://code.haskell.org/xmonad
 Copying patch 941 of 941... done!
 Applying patch 941 of 941... done.
 Finished getting.
 [brent@archimedes:~/haskell/xmonad-dev-tut]$ darcs get http://code.haskell.org/XMonadContrib
 Copying patch 1292 of 1292... done!
 Applying patch 1292 of 1292... done.
 Finished getting.
 [brent@archimedes:~/haskell/xmonad-dev-tut]$ ls
...
just issue the usual incantations:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut]$ cd xmonad/
 [brent@archimedes:~/haskell/xmonad-dev-tut/xmonad]$ cabal install
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/xmonad]$ cd ../XMonadContrib/
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ cabal install
...
Now restart xmonad (run <code>touch ~/.xmonad/xmonad.hs</code>, then hit mod-q).  Note, if you are upgrading from a previous version of xmonad, you may need to first unregister the old packages with ghc.  Type <code>ghc-pkg list xmonad</code> and <code>ghc-pkg list xmonad-contrib</code> at a prompt; if multiple versions are listed you need to unregister all but the newest.  For example:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut]$ ghc-pkg unregister xmonad-0.5 --user
 Saving old package config file... done.
 Writing new package config file... done.
 [brent@archimedes:~/haskell/xmonad-dev-tut]$ ghc-pkg unregister xmonad-contrib-0.5 --user
 Saving old package config file... done.
 Writing new package config file... done.
...
So, let's add that module!  We'll call it XMonad.Actions.HelloWorld, so it needs to go in the XMonad/Actions/ subdirectory of the XMonadContrib repository:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ $EDITOR XMonad/Actions/HelloWorld.hs &
...
OK, that's a good start.  Now let's go to a prompt and see what we've done so far:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs whatsnew
 No changes!
...
Huh? No changes?  Well, that's because we haven't added HelloWorld.hs to the repository yet, so darcs ignores it.  Let's do that now:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs add XMonad/Actions/HelloWorld.hs 
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs whatsnew
 {
...
Our new module is great, but it won't get compiled as part of the xmonad-contrib library unless we add a reference to it in xmonad-contrib.cabal.
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ $EDITOR xmonad-contrib.cabal
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs whatsnew -u xmonad-contrib.cabal 
...
* Use @ symbols to surround code which should be printed in a <code>verbatim style</code>.
...
To generate the documentation, first rebuild the xmonad-contrib library, then use <code>cabal haddock</code>:
 ...
 Configuring xmonad-contrib-0.9.1...
 ...blah blah...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ cabal haddock
 Preprocessing library xmonad-contrib-0.9.1...
...
In this case, to view the generated documentation, you should point your browser to something like <code>file:///path/to/XMonadContrib/dist/doc/html/xmonad-contrib/index.html</code>, then navigate to the documentation for your module. Check that the formatting, links, and so on are correct.  For example, when first writing this tutorial, I forgot to escape the double quotes around "hello world" in the comments for the <hask>helloWorld</hask> function, which therefore showed up as a link to the (nonexistent) "hello world" module, instead of showing literal double quote characters.

If you make any changes to your documentation, you can simply rerun <code>cabal haddock</code> to regenerate; there's no need to rebuild everything as long as you only changed documentation. 
...
<code>cabal haddock --hyperlink-source</code>
...
The first step is to record our changes as one or more darcs patches.  First, let's see what we've changed:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs whatsnew -s
...
Now, if you have a mail agent configured correctly, all you have to do is type <code>darcs send</code> at the prompt, and it will send the patches you select as attachments to an e-mail to the xmonad list (<code>xmonad@haskell.org</code>).  Alternatively, you can <code>darcs send</code> to an output file with the <code>-o</code> option:
 ...
 [brent@archimedes:~/haskell/xmonad-dev-tut/XMonadContrib]$ darcs send -o ../helloWorld.dpatch
...
Questions? Suggestions? Confusions? Edit this tutorial yourself (it's a wiki, after all!), send an email to the [http://www.haskell.org/mailman/listinfo/xmonad xmonad mailing list], or join the <code>#xmonad</code> channel on irc.freenode.net.  
